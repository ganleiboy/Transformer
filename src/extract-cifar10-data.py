...
# refer：https://blog.csdn.net/zcyzcyjava/article/details/126827754
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
import numpy as np
import pickle
import imageio
...
list = ["airplane", "automobile", "bird", "cat",
...
for j in range(1, 6):
    dataName = "../data/cifar-10-batches-py/data_batch_" + \
        str(j)
    ...
    for i in range(0, 10000):
        ...
        picName = 'train/' + list[Xtr['labels'][i]] + \
            '/' + str(i + (j - 1) * 10000) + '.jpg'  # png
...
# 生成测试集图片
# 同dataName一样也是自己的文件所在路径
testXtr = unpickle(
    "../data/cifar-10-batches-py/test_batch")
for i in range(0, 10000):
    img = np.reshape(testXtr['data'][i], (3, 32, 32))
    img = img.transpose(1, 2, 0)
    picName = 'val/' + list[testXtr['labels'][i]] + '/' + str(i) + '.jpg'
    imageio.imwrite(picName, img)
