# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# ViT其实只有Transformer模型的encoder部分，本代码和论文基本是一样的
...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, image_size=224, patch_size=16, in_channel=3, embed_dim=768, dropout=0.) -> None:
        ...
        self.patch_embedding = nn.Conv2d(in_channels=in_channel,
                                         ...
                                         stride=patch_size)
        # 位置嵌入，可学习
        # Parameter和Tensor的区别，https://blog.csdn.net/hxxjxw/article/details/107904012
        self.position_embedding = nn.Parameter(torch.Tensor(1, num_patches+1, embed_dim), requires_grad=True)
    ...
    def forward(self, x):
        ...
        # 增加分类token
        x = torch.cat((cls_tokens, x), dim=1)  # 将分类token增加到最前面, [2, 197, 768]
        # 位置嵌入
...
class VisualTransformer(nn.Module):
    def __init__(self,
        ...
        self.classifier = Head(embed_dim, num_classes)

    def forward(self, x):
        # input x: [2, 3, 224, 224]
        x = self.patch_embedding(x)  # [2, 197, 768]
        x = self.encoder(x)  # [n, h'*w'+1, c]，[2, 197, 768], 此处的c就是embed_dim，类似于CNN中的channel
